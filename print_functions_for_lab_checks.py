...
def check_calculating_results(results_dic, results_stats_dic):
    ...
    if results_stats_dic is None:
        ...
        # Calculating pct_corr_dog
        if n_pet_dog > 0:
            pct_corr_dog = (n_class_cdog / n_pet_dog) * 100.0                                            
        else:
            pct_corr_dog = 0.0 
          
        # ** Original *** pct_corr_dog = ( n_class_cdog / n_pet_dog )*100
        
        # Calculating pct_corr_notdog
        if n_pet_notd > 0: 
            pct_corr_notdog = ( n_class_cnotd / n_pet_notd )*100
        else:
            pct_corr_notdog = 0.0 
            
        # ** Original *** pct_corr_notdog = ( n_class_cnotd / n_pet_notd )*100
        
        # Calculating pct_corr_breed
        if n_pet_dog > 0:
